...
class Solution {
    public int maxProfit(int[] prices) {
        ...
        for(int i =0 ;i< prices.length;i++){
            secondRelease = Math.max(secondRelease, prices[i]+secondBuy);
            secondBuy =Math.max(secondBuy,firstRelease-prices[i]);
            firstRelease = Math.max(firstRelease, prices[i]+firstBuy);
